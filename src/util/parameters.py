...
class ParameterLoader:
    ...
    def _load_base_parameters(self) -> None:
        ...
        ignored_lists_raw = os.getenv("IGNORED_LISTS")
        if (
            ignored_lists_raw
            and isinstance(ignored_lists_raw, str)
            and len(ignored_lists_raw.strip()) > 0
        ):
            ignored_lists = ignored_lists_raw.split(",")

        custom_date: Optional[datetime] = None
        # custom_date = datetime(2025, 10, 8, 0, 0, 0, tzinfo=tzinfo)
