...
function clean_cache() {
    ...
    rm -rf .pytest_cache .mypy_cache .cache dist build *.egg-info htmlcov .coverage nonexistent.json logs tests/data /tmp/data/market_enriched_data.json data/market_enriched_data.json
...
function code_check() {
    ...
    # Filter: keep only existing paths (files/dirs). Warn on missing ones.
    local existing_paths=()
    for p in "${paths[@]}"; do
        if [ -e "$p" ]; then
            existing_paths+=("$p")
        else
            echo -e "${GREEN}ℹ️  Skipping missing path: ${p}${NC}"
        fi
    done

    echo -e "${GREEN}📁 Using paths (existing only): ${existing_paths[*]:-(none)}${NC}"
    ...
    if [ -d "src" ]; then
        ...
        echo -e "${GREEN}ℹ️  Skipping pydocstringformatter: src/ not found.${NC}"
    fi

    # If nothing exists, stop gracefully.
    if [ ${#existing_paths[@]} -eq 0 ]; then
        echo -e "${GREEN}✅ No valid paths to check. Nothing to do.${NC}"
        return 0
    fi
    ...
    black "${existing_paths[@]}" || rc=$((rc+1))

    echo -e "${GREEN}🛡️  Running bandit to detect potential security issues...${NC}"
    bandit -r "${existing_paths[@]}" || rc=$((rc+1))

    echo -e "${GREEN}🔧 Running isort to sort and organize imports...${NC}"
    isort "${existing_paths[@]}" || rc=$((rc+1))

    echo -e "${GREEN}🧹 Running autoflake to remove unused imports and variables...${NC}"
    autoflake --remove-all-unused-imports --remove-unused-variables --in-place --recursive "${existing_paths[@]}" || rc=$((rc+1))

    echo -e "${GREEN}🔍 Running pylint for static code analysis and linting...${NC}"
    # Make project root importable so "src.*" resolves during static analysis.
    PYTHONPATH="${PYTHONPATH:-}:." pylint \
        --persistent=no \
        --rcfile=.pylintrc \
        --ignore-paths='src/_old/.*|tests/_old/.*' \
        "${existing_paths[@]}" || rc=$((rc+1))

    if [ $rc -eq 0 ]; then
        echo -e "${GREEN}✅ Code quality checks completed.${NC}"
    else
        echo -e "${RED}⚠️  One or more tools reported issues. Missing paths were ignored as requested.${NC}"
        return 1
    fi
}
...
case ${1:-} in
    ...
    test)
        ...
        if [ -d "tests" ]; then
            ...
            echo -e "${GREEN}ℹ️  Skipping tests: tests/ not found.${NC}"
        fi
        ;;
    *)
        echo -e "${GREEN}Usage:"
